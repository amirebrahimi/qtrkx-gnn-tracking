...
def qc10_pqc_two_design(circuit, qubits, n_layers=1, n_qubits=4):
    ...
    # print(circuit)
###############################################################################
def qc1021_pqc(circuit, qubits, n_layers=1, n_qubits=4):
    print("QC103_PQC")
    params = sympy.symbols('theta:{}'.format(n_qubits*(1+n_layers)))
    for i, qubit in enumerate(qubits):
        #symbol = sympy.Symbol('theta_{}'.format(i+1))
...
def qc102_pqc(circuit, qubits, n_layers=1, n_qubits=4):
    ...
    print(circuit)
###############################################################################
def qc104_pqc(circuit, qubits, n_layers=1, n_qubits=4):
    print("QC104_PQC")
    params = sympy.symbols('theta:{}'.format(n_qubits*(1+n_layers)))
    prep_and_U = qc103_pqc(circuit, qubits, n_layers, n_qubits)
    circuit.append(prep_and_U)
    U_dagger = (prep_and_U[1:])**-1
    ### TODO: ADD PARAM RESOLVER ###
    for d in range(n_layers - 1):
        prep_and_U_circuit = prep_and_U
    ...
    print(circuit)
###############################################################################
def qc102_old_pqc(circuit, qubits, n_layers=1, n_qubits=4):
    #print("QC102_PQC")
    params   = sympy.symbols('theta:{}'.format(n_qubits*(1+n_layers)))
    for i, qubit in enumerate(qubits):
        #symbol = sympy.Symbol('theta_{}'.format(i+1))
